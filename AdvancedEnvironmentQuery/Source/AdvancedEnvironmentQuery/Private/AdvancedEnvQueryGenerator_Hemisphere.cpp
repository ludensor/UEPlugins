...
#include "AdvancedEnvQueryGenerator_Hemisphere.h"
...
UAdvancedEnvQueryGenerator_Hemisphere::UAdvancedEnvQueryGenerator_Hemisphere(const FObjectInitializer& ObjectInitializer)
...
float UAdvancedEnvQueryGenerator_Hemisphere::GetArcBisectorAngle(FEnvQueryInstance& QueryInstance) const
{
...
int32 UAdvancedEnvQueryGenerator_Hemisphere::GetItemCount(float SphereRadius, float ItemSpace, int32 RingCount, float CircleAngleDelta) const
{
	int32 ItemCount = 0;
...
bool UAdvancedEnvQueryGenerator_Hemisphere::IsAngleAllowed(float TestAngleRadians, float BisectAngleDegrees, float AngleRangeDegrees) const
{
	if (bDefineArc)
	{
...
void UAdvancedEnvQueryGenerator_Hemisphere::GenerateItems(FEnvQueryInstance& QueryInstance) const
{
...
void UAdvancedEnvQueryGenerator_Hemisphere::StoreNavPoints(const TArray<FNavLocation>& Points, FEnvQueryInstance& QueryInstance) const
{
...
FText UAdvancedEnvQueryGenerator_Hemisphere::GetDescriptionTitle() const
{
...
FText UAdvancedEnvQueryGenerator_Hemisphere::GetDescriptionDetails() const
{
...
#if WITH_EDITOR
void UAdvancedEnvQueryGenerator_Hemisphere::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	if (PropertyChangedEvent.Property)
	{
		const FName PropName = PropertyChangedEvent.MemberProperty->GetFName();
		if (PropName == GET_MEMBER_NAME_CHECKED(UAdvancedEnvQueryGenerator_Hemisphere, ArcAngle))
		{
		...
		else if (PropName == GET_MEMBER_NAME_CHECKED(UAdvancedEnvQueryGenerator_Hemisphere, ConeVerticalAngle))
		{
			ConeVerticalAngle.DefaultValue = FMath::Clamp(ConeVerticalAngle.DefaultValue, 0.0f, 90.0f);
		}
		else if (PropName == GET_MEMBER_NAME_CHECKED(UAdvancedEnvQueryGenerator_Hemisphere, SpaceBetween))
		{
			SpaceBetween.DefaultValue = FMath::Max(1.0f, SpaceBetween.DefaultValue);
		}
		else if (PropName == GET_MEMBER_NAME_CHECKED(UAdvancedEnvQueryGenerator_Hemisphere, NumberOfRings))
		{
